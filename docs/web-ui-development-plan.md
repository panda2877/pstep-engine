...
- [ ] 配置 Tailwind CSS 和设计系统（颜色、字体、间距）
- [ ] 实现 AppLayout 整体布局
- [ ] 实现 Topbar 组件
- [ ] 实现 FnBar 组件
- [ ] 实现响应式布局基础

**交付物**：可运行的空壳项目，布局与原型一致

### Phase 2：Agent 管理模块（1 周）

**目标**：完成 Agent 列表和会话管理

- [ ] 后端：新增 `agents` 表和 CRUD API
- [ ] 后端：改造 `sessions` 表，增加 `agent_id` 字段
- [ ] 前端：实现 AgentBar 组件
- [ ] 前端：实现 Agent 折叠/展开交互
- [ ] 前端：实现会话列表和切换
- [ ] 前端：实现新建会话功能

**交付物**：Agent 列表可正常显示和切换

### Phase 3：聊天核心功能（1.5 周）

**目标**：完成消息收发和流式输出

- [ ] 后端：改造 `/api/chat` 接口，支持 agentId
- [ ] 后端：实现消息搜索 API
- [ ] 前端：实现 MessageArea 组件
- [ ] 前端：实现消息列表渲染（用户/Agent 消息）
- [ ] 前端：实现 SSE 流式消息接收
- [ ] 前端：实现输入框和发送功能
- [ ] 前端：实现流式输出动画效果
...
- [ ] 后端：新增用户管理 API
- [ ] 后端：暴露记忆管理 API
- [ ] 前端：实现 HelperPanel 组件
- [ ] 前端：实现 User/Soul/Memory 三个标签页
- [ ] 前端：实现 SearchModal 组件
- [ ] 前端：实现消息搜索功能

**交付物**：辅助面板和搜索功能完整

### Phase 5：移动端适配和优化（0.5 周）
...
|-----|------|---------|
| Phase 1 | 1 周 | 基础布局框架 |
| Phase 2 | 1 周 | Agent 管理功能 |
| Phase 3 | 1.5 周 | 聊天核心功能 |
| Phase 4 | 1 周 | 辅助面板和搜索 |
| Phase 5 | 0.5 周 | 移动端适配 |
| Phase 6 | 1 周 | 联调部署 |
| **总计** | **6 周** | 完整 Web UI |
